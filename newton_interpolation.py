...
# x: 0, 1, 2, 3
# y: 1, 2, 4, 7
# 1. Use Newton's Forward Interpolation to find y(0.5).
# 2. Use Newton's Backward Interpolation to find y(2.5).
...
# 2. Generate the difference table (forward or backward).
...
def newton_forward(x_data, diff_table, value):
    ...
    for i in range(1, n):
        ...
        y_val += (u_term / factorial(i)) * diff_table[0, i]
...
def newton_backward(x_data, diff_table, value):
    ...
    for i in range(1, n):
        ...
        y_val += (u_term / factorial(i)) * diff_table[n - 1 - i, i]
...
if __name__ == "__main__":
    ...
    print("\n--- Output ---")
    print(f"Forward interpolation for y({val_forward}): {result_forward:.6f}")
    print(f"Backward interpolation for y({val_backward}): {result_backward:.6f}")
