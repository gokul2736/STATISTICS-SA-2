...
Define the derivative of the function g(x) = f'(x) = 3x^2 - 1

Choose an initial guess (x0) and a tolerable error (e).

Start the iteration:

Calculate x1 = x0 - f(x0) / g(x0)

Check if abs(x1 - x0) is less than the tolerable error e.

If yes, x1 is the root. Stop.

If no, set x0 = x1 and repeat from step 5.
...
Arrange the system of equations into a diagonally dominant form (if possible).

Express each variable from one equation (e.g., x from eq1, y from eq2, z from eq3).
...
Start with an initial guess [x, y, z] = [0, 0, 0].

In each iteration, calculate the new value for x, y, and z using the most recent values of the other variables.

Repeat until the solution converges (i.e., the change between iterations is below a specified tolerance).
...
Create a set of n data points (x, y).

Generate the difference table.

For Forward Interpolation (estimating near the start of the table):
a.  Calculate u = (value - x[0]) / (x[1] - x[0])
b.  Apply the formula: y_val = y[0] + u*delta_y[0] + (u*(u-1)/2!)*delta_2_y[0] + ...

For Backward Interpolation (estimating near the end of the table):
a.  Calculate u = (value - x[n-1]) / (x[1] - x[0])
b.  Apply the formula: y_val = y[n-1] + u*nabla_y[n-1] + (u*(u+1)/2!)*nabla_2_y[n-1] + ...
...
Create a set of n data points (x_data, y_data).

Define the point value at which to interpolate.

Initialize the interpolated result y_val to 0.

Loop for i from 0 to n-1 (for each point):
a.  Initialize the Lagrange basis polynomial L_i to 1.
b.  Loop for j from 0 to n-1:
c.  If i is not equal to j:
L_i = L_i * (value - x_data[j]) / (x_data[i] - x_data[j])
d.  Add the weighted value to the total: y_val = y_val + y_data[i] * L_i
...
# x: 0, 1, 3
# y: 1, 2, 10
# (These points lie on the polynomial y = x^2 + 1. We will pretend we don't know this.)
...
if __name__ == "__main__":
    ...
    y_data = np.array([1, 2, 10], dtype=float)
    ...
    # Verification with the actual polynomial y = x^2 + 1
    actual = (value_to_find**2) + 1
    print(f"The exact value from the polynomial y=x²+1 is {actual:.4f}.")
...
Set initial values x0 = 0, y0 = 1.

Set step size h = 0.1 and target x_target = 1.

Calculate the number of steps: n = (x_target - x0) / h.

Start iteration from i = 0 to n-1:
...
The final y_next is the approximation for y(1).
...
Set initial values x0 = 0, y0 = 1, h = 0.1, and x_target = 1.
